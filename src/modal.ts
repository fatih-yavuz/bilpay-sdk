...
const animation = $('#successAnimation', modal);
animation.css({
    position: 'absolute'
});

(<any>modal).hide = () => {
    (<any>modal).isOpen = false;
    hideLoading();
    modal.css(hideModal)
};
...
const showLoading = () => {
    $('#error', modal).hide();
    $('#loading', modal).addClass('bilpay-loading');
    $('#loading', modal).show();
};


(<any>modal).show = () => {
    (<any>modal).isOpen = true;
    resetCheck();
    modal.css(showModal);
};
